...
const parseVersion = (version, container) => {
  try {
    ...
    } else if (container.includes('tron')) {
      return 'Tron ' + version
    } else if (container.includes('eos')) {
      return 'Eos ' + version
    } else {
      if (typeof version === 'string') {
        version = JSON.parse(version);
...
exports.getWalletInfos = async (container, rpcPort) => {
  try {
    ...
    if (container.includes('parity')) {
      ...
      rpcCmd = `${clis['stellar-node']} --version`;
      versionCmd = `${clis['stellar-node']} --version`;
    } else if (container.includes('trx-node')) {
      rpcCmd = `${clis['trx-node']} -V`;
      versionCmd = `${clis['trx-node']} -V`;
    } else if (container.includes('eos-node')) {
      rpcCmd = `${clis['eos-node']} -V`;
      versionCmd = `${clis['eos-node']} -V`;
    } else {
