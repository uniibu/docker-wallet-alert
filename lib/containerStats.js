...
exports.getWalletInfos = async (container, rpcPort) => {
  try {
    ...
    if (container.includes('parity')) {
      rpcCmd = `curl -s --data '{"method":"eth_blockNumber","params":[],"id":1,"jsonrpc":"2.0"}' -H "Content-Type: application/json" -X POST localhost:${rpcPort}`;
    } else if (container.includes('ripple')) {
      rpcCmd = `curl -s --data '{"method":"blockNumber","params":[],"id":1,"jsonrpc":"2.0"}' -H "Content-Type: application/json" -X POST localhost:${rpcPort}`;
    } else {
...
exports.getContainers = async () => {
  ...
  const c = {};
  for (let l of list) {
    c[l] = process.env[l.replace('-node', '').toUpperCase()];
  }
  return c;
};